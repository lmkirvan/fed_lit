...
dates_df <- purrr::map(dates_df, "result") |> purrr::discard( \(x) any(is.na(x))) 
...
nominations <- nominations |> 
  tidyr::separate_wider_delim(
    value
    , delim = " on "   
    , names = c("nominator", "nomination date")
    , too_many = "merge"
    ) 
...
add_court_id <- function(df){
  df |> 
    group_by(id) |> 
    mutate(
      court_number = row_number()
      , full_id = paste0(id, "-", court_number)
      ) |> 
    select(-court_number)
}

df <- add_court_id(df)
courts <- add_court_id(courts)
terminations <- add_court_id(terminations)
nominations <- add_court_id(nominations)
